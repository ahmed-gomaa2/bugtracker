...
module.exports = app => {
    app.put('/workspace/edit-title', auth, userOwnTask, (req, res) => {
        const data = req.body;
        const user_id = req.user;
        const changeTitleQuery = 'UPDATE tasks SET title = ? WHERE id = ?';
        connection.query(changeTitleQuery, [data.title, data.id], (changeTitleError, changeTitleRes) => {
            if(changeTitleError) {
